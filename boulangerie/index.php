<?php
/**
 * index.php — Page d'accueil Boulangerie Ouahabi
 * Design: Editorial / Magazine chic
 */
require_once 'db.php';
...
// Mini cards pour le hero (3 premiers)
$miniProduits = array_slice($topProduits, 0, 3);

function catIcon(string $n): string {
    $n = mb_strtolower($n);
    if (str_contains($n,'pain') || str_contains($n,'baguette')) return '🍞';
    if (str_contains($n,'croissant') || str_contains($n,'viennois')) return '🥐';
    if (str_contains($n,'gâteau') || str_contains($n,'gateau') || str_contains($n,'cake')) return '🎂';
    if (str_contains($n,'pâtisserie') || str_contains($n,'patisserie')) return '🍰';
    if (str_contains($n,'sandwich')) return '🥪';
    if (str_contains($n,'brioche')) return '🫓';
    return '🥖';
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Boulangerie Ouahabi — Artisan Boulanger · Casablanca</title>
...
<section class="hero">
  <!-- Texte gauche -->
  <div class="hero-left">
    <p class="hero-eyebrow">Depuis 1985 — Casablanca</p>
  ...
  <div class="hero-right">
    <div style="width:100%;height:100%;background:linear-gradient(135deg,#C9A97A,#6B3F22);display:flex;align-items:center;justify-content:center;font-size:10rem;">🥖</div>
    <div class="hero-right-overlay"></div>

    <!-- Mini product cards overlay -->
    <?php if (!empty($miniProduits)): ?>
    <div class="hero-mini-cards">
      <?php foreach ($miniProduits as $mp): ?>
      <div class="hero-mini-card">
        <div class="hero-mini-img">
          <?php if (!empty($mp['image']) && file_exists('images/'.$mp['image'])): ?>
            <img src="images/<?= htmlspecialchars($mp['image']) ?>" alt="">
          <?php else: ?>
            🥖
          <?php endif; ?>
        </div>
        <div class="hero-mini-info">
          <div class="mini-name"><?= htmlspecialchars($mp['nom_produit']) ?></div>
          <div class="mini-price"><?= number_format($mp['prix'],2) ?> MAD</div>
        </div>
      </div>
      <?php endforeach; ?>
    </div>
  ...
  <div class="container">
    <div class="about-layout">
      <div class="about-text-col">
        ...
        <h2 class="section-title">La passion du pain<br><em>depuis 40 ans</em></h2>
        <p>Fondée en 1985 par la famille Ouahabi, notre boulangerie perpétue les traditions artisanales marocaines tout en intégrant les meilleures techniques françaises de boulangerie.</p>
        ...
        <ul class="about-list">
          ...
          <li>Cuisson au four à bois traditionnel</li>
      ...
      <div class="about-images">
        <div class="about-img tall" style="background:var(--beige);display:flex;align-items:center;justify-content:center;font-size:6rem;">🍞</div>
        <div class="about-img" style="background:var(--beige-warm);display:flex;align-items:center;justify-content:center;font-size:4rem;">🥐</div>
        <div class="about-img" style="background:#E8D0B0;display:flex;align-items:center;justify-content:center;font-size:4rem;">🎂</div>
      </div>
    </div>
  </div>
</section>
    ...
    <div class="cat-grid">
      <?php foreach ($categories as $cat): ?>
        <a href="produits.php?categorie=<?= $cat['id'] ?>" class="cat-card">
          <div class="cat-icon"><?= catIcon($cat['nom_categorie']) ?></div>
