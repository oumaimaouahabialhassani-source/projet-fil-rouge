...
// On prend les 3 premiers produits pour les afficher sur le hero
$miniProduits = array_slice($topProduits, 0, 3);

// Petit icone selon le nom de la categorie
function catIcon($nom) {
    $nom = strtolower($nom);
    if (strpos($nom, 'pain') !== false) return '🍞';
    if (strpos($nom, 'viennois') !== false) return '🥐';
    if (strpos($nom, 'gateau') !== false || strpos($nom, 'tarte') !== false) return '🎂';
    if (strpos($nom, 'patisserie') !== false) return '🍰';
    return '🥖';
}
?>
<!DOCTYPE html>
